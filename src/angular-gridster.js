...
(function(root, factory) {
	...
	.controller('GridsterCtrl', ['gridsterConfig', '$rootScope', '$timeout',
		function(gridsterConfig, $rootScope, $timeout) {
				...
				for (var rowIndex = 0; rowIndex < this.maxRows; ++rowIndex) {
					for (var colIndex = 0; colIndex < this.columns; ++colIndex) {
						...
						if (items.length === 0 && this.canItemOccupy(item, rowIndex, colIndex)) {
							...
							$rootScope.$broadcast('gridster:itemPlaced', item);
							return;
						}
					}
				}
			...
			return {
				...
				link: function(scope, $el, attrs, controllers) {
					...
					function positionChanged() {
						...
						item.setPosition(item.row, item.col);
						// var a = scope, b = attrs, c = controllers;
						// console.log ('gridster item placed!');
						// console.log ('item = ', item);
						// console.log ('widget = ', scope.$eval(attrs.gridsterItem));
						// console.log ('col = ', item.col);
						// console.log ('row = ', item.row);
						$rootScope.$broadcast('gridster:itemPositionChanged', {
							gridsterItem: item,
							item: scope.$eval(attrs.gridsterItem),
							col: item.col,
							row: item.row
						});
